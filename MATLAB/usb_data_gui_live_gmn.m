...
% --- Helper Functions ---
    function fig = create_gui()
        ...
        % Status Text Box
        handles.statusText = uicontrol('Style', 'text', ...
            'Tag', 'statusText',...
            'String', 'Idle', ...
            'Position', [380 30 300 30], ...
            'FontSize', 12);
        % Display packet count
        handles.packetCounter = uicontrol('Style', 'text', ...
            'Tag', 'packetCounter',...
            'String', 'Packets: 0', ...
    ...
    function stopCallback(~, ~)
        ...
        if handles.isRunning
            if ~isempty(handles.s)
                %whos('handles.s'); % Debug: Check type of handles.s
                if isvalid(handles.s) % Check if handles.s is a valid object
                    fprintf(handles.s, 'T'); % Send 'T' command to STM32
                else
                    disp('Warning: handles.s is not a valid object.');
                end
            end
            handles.isRunning = false;
            %whos('handles.statusText'); % Debug: Check type of handles.statusText
            if isvalid(handles.statusText) % Check if handles.statusText is a valid handle
        ...
        if ~isempty(handles.s)
            ...
            if isvalid(handles.s) % Check if handles.s is a valid object
                clear handles.s;
                 ...
                 disp('Warning: handles.s is not a valid object during closeGUI.');
            end
        end
        %whos('fig'); % Debug: Check type of fig
        if isvalid(fig) % Check if fig is a valid handle
            delete(fig);
        else
...
function data_acquisition_loop()
    ...
    while handles.isRunning
            ...
            while numel(handles.byteBuffer) >= handles.packetSize
                ...
                if ~isempty(headerIdx)
                    if (numel(handles.byteBuffer) >= headerIdx + handles.packetSize - 1)
                        ...
                        % Assuming time_data is the first value in values
                        time_data_ms = double(values(1)); % Time in ms (assuming first value is time)
                        sensor_values = double(values(2:end)'); % Sensor values
                        if length(sensor_values) == 4 % Ensure we have 4 sensor values to match plot lines
                            handles.dataBuffer = [handles.dataBuffer; [packet_info, packet_count, time_data_ms, sensor_values]];
                            packet_count = packet_count + 1;
                        else
                            disp('Warning: Incomplete sensor data in packet.');
                        end
        ...
        catch serial_error % Corrected variable name
    ...
    if ~isempty(handles.s)
        ...
        if isvalid(handles.s) % Check if handles.s is a valid object
            ...
            handles.s = [];
             ...
             disp('Warning: handles.s is not a valid object after loop.');
        end
    end
...
function updatePlots(handles)
    ...
    if size(handles.dataBuffer, 1) > 10
        ...
        % Time data is in column 3 (after packet_info and packet_count)
        time_data = plotData(:, 3);

        % Update each data line - Corrected indexing to match dataBuffer structure
        %whos('handles.dataLines'); % Debug: Check type of handles.dataLines
        if isvalid(handles.dataLines(1))
            set(handles.dataLines(1), 'XData', time_data, 'YData', plotData(:, 3)); % Time
        end
        if isvalid(handles.dataLines(2))
            set(handles.dataLines(2), 'XData', time_data, 'YData', plotData(:, 4)); % Panasonic
        end
        if isvalid(handles.dataLines(3))
            set(handles.dataLines(3), 'XData', time_data, 'YData', plotData(:, 5)); % Load Cell
        end
        if isvalid(handles.dataLines(4))
            set(handles.dataLines(4), 'XData', time_data, 'YData', plotData(:, 6)); % Set RPM
        end
        if isvalid(handles.dataLines(5))
            set(handles.dataLines(5), 'XData', time_data, 'YData', plotData(:, 7)); % Current Speed
        end
        ...
        if ~isempty(time_data)
            ...
            % Auto-adjust Y axis
            all_y_data = plotData(:, 4:7); % Corrected to columns 4:7 for sensor data
            if ~isempty(all_y_data)
                ylim(handles.dataPlot, [min(all_y_data(:)), max(all_y_data(:))]);
            end
        end
        % Update packet counter display
        if ~isempty(plotData) % Check if plotData is not empty before accessing it
